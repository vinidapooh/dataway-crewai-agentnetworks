...
def build_crew(state_snapshot: str) -> Crew:
    ...
    topic_scout = Agent(
        ...
        backstory="""You monitor AI, analytics, data engineering and adjacent technical developments. """
                  """You look for meaningful signals rather than generic hype. Compare every discovery """
                  """with the persistent Dataway state and prefer updating existing topics when appropriate.""",
    ...
    analyst_lens = Agent(
        ...
        backstory="""You understand analysts who know Excel, SQL, Python, BI, automation and basic data engineering. """
                  """For each candidate ask what the analyst already knows, what is genuinely new, what problem """
                  """the technology solves, and what prerequisites are needed.""",
        llm=llm,
        verbose=True,
    )
    ...
    source_scout = Agent(
        ...
        backstory="""Prefer primary documentation, papers, talks, strong technical explanations and practical """
                  """material. Explain why each source is useful and distinguish primary evidence from commentary.""",
        tools=research_tools,
        llm=llm,
        verbose=True,
    )
    learning_researcher = Agent(
        ...
        backstory="""Teach the creator first, not the audience. Explain progressively, connect concepts to analyst """
                  """knowledge, identify misconceptions, and explicitly flag where analogies break down. """
                  """Prefer a table or document demonstration where appropriate.""",
        tools=research_tools,
        llm=llm,
        verbose=True,
    )
    ...
    scout_task = Task(
        description="""Run a daily discovery pass for Dataway. Current persistent state is below:\n\n{state_snapshot}\n\n"
                     """Find meaningful developments across AI, analytics, data engineering and adjacent fields. """
                     """Search current web sources when tools are available. Identify new signals and updates to existing topics. """
                     """Do not write scripts. Return a concise, structured candidate list with evidence and URLs where available.""",
    ...
    lens_task = Task(
        description="""Evaluate the discovered signals through the Dataway analyst lens. Score each for analyst relevance, """
                     """learning value, current interest, evergreen value, demo potential and prerequisite burden. """
                     """Recommend research-now, watch, defer or reject.\n\nPersistent state:\n{state_snapshot}""",
    ...
    learning_task = Task(
        ...
        agent=learning_researcher,
        context=[source_task],
    )
    content_task = Task(
        description="""For topics that are sufficiently understood, create a content plan. If a topic is too uncertain or lacks a """
                     """solid learning foundation, mark it as not-ready rather than forcing a content plan. For ready topics provide """
                     """one focal question, whiteboard sequence, dataset/document demo, private talking points, title angles and X angle.""",
    ...
    eval_task = Task(
        ...
        agent=evaluator,
        context=[scout_task, lens_task, curriculum_task, source_task, learning_task, content_task],
    )

    return Crew(
        agents=[topic_scout, analyst_lens, curriculum_mapper, source_scout, learning_researcher, content_architect, evaluator],
        tasks=[scout_task, lens_task, curriculum_task, source_task, learning_task, content_task, eval_task],
